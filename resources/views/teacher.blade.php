  ...
  <main class="flex-1 p-4">
    <div id="view-dashboard" class="view-content space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="card bg-white rounded-xl p-5 shadow-lg border-b-4 border-indigo-500">
          ...
          <strong id="coursesTaught" class="text-3xl font-extrabold text-indigo-600">{{$grupos->count()}}</strong>
          ...
          <div id="teacherCourseList" class="space-y-2 text-sm">
            @foreach($grupos as $grupo)
              <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="font-medium text-indigo-700">{{$grupo['curso']['nombre']}} ({{ucfirst($grupo['curso']['tipo'])}})</span>
                <span class="text-xs text-gray-500">Alumnos: {{$grupo['registros']['cantidad']}} | P. Parcial: {{$grupo['registros']['promedio_parcial']}} | P. Continua: {{$grupo['registros']['promedio_continua']}}</span>
              </div>
            @endforeach
          </div>
        </div>
    ...
    <div id="view-grades-input" class="view-content space-y-6 hidden">
      ...
      <div id="courseCardSelector" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          @foreach($grupos as $grupo)
          <div onclick="selectCourseForManagement({{$grupo['curso']['id']}}, '{{$grupo['curso']['nombre']}}')" class="p-6 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl shadow-lg cursor-pointer hover:shadow-xl hover:scale-105 transition text-white">
            <h3 class="text-xl font-bold mb-2">{{$grupo['curso']['nombre']}}</h3>
            <p class="text-sm opacity-90">{{$grupo['registros']['cantidad']}} Alumnos</p>
            <p class="text-xs opacity-75 mt-1">P. Parcial: {{$grupo['registros']['promedio_parcial']}} | P. Continua: {{$grupo['registros']['promedio_continua']}}</p>
          </div>
          @endforeach
        </div>
      </div>
    ...
    <div id="scheduleModal" class="fixed inset-0 bg-gray-600 bg-opacity-75 hidden flex items-center justify-center z-50 p-4" onclick="closeScheduleModal(event)">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-100" onclick="event.stopPropagation()">
        <div class="p-6 border-b flex justify-between items-center">
          <h3 class="text-xl font-bold text-gray-800">Reservar Clase / Tutoría</h3>
          ...
          <form id="scheduleForm" onsubmit="event.preventDefault(); saveNewScheduleEvent()">
            ...
            <div class="mb-4">
              ...
              <select id="eventCurso" required
                ...
                @foreach($grupos as $grupo)
                <option value="{{$grupo['curso']['id']}}">{{$grupo['curso']['nombre']}}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input type="date" id="eventDate" required class="mt-1 p-3 border border-gray-300 rounded-lg w-full focus:ring-indigo-500 focus:border-indigo-500">
              </div>
              <div>
                <label for="eventLocation" class="block text-sm font-medium text-gray-700">Aula / Ubicación:</label>
                <input type="text" id="eventLocation" required class="mt-1 p-3 border border-gray-300 rounded-lg w-full focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ej: Aula 302, Zoom, Oficina">
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <input type="time" id="eventStart" required class="mt-1 p-3 border border-gray-300 rounded-lg w-full focus:ring-indigo-500 focus:border-indigo-500">
              </div>
              <div>
                <label for="eventEnd" class="block text-sm font-medium text-gray-700">Hora de Fin:</label>
                <input type="time" id="eventEnd" required class="mt-1 p-3 border border-gray-300 rounded-lg w-full focus:ring-indigo-500 focus:border-indigo-500">
              </div>
            </div>
            ...
            <div class="mt-6 text-right">
              ...
              <button type="submit" class="px-4 py-2 rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition">
                Reservar Hora
              </button>
            </div>
          </form>
